    ...
    if (window) {
      window.fastPush = (url, as = url, options = {}) => 
        options.shallow 
          ...
          : (prefetched.add(normalize(as || url)), window.location.href = as || url, Promise.resolve(true));

      window.fastReplace = (url, as = url, options = {}) =>
        options.shallow
          ? replace(url, as, { ...options, scroll: false })
          : (prefetched.add(normalize(as || url)), window.location.replace(as || url), Promise.resolve(true));
    }
  }, [push, replace]);
};
